...
requirements_qa = """
        ...
        RFP and a detailed list of all the specified requirements. When listing the requirements, make sure to clearly indicate each 
...
roles_extraction_qa = """
                       Thoroughly review the RFP document to identify any explicitly stated or implied key roles and corresponding criteria required for the project team. Extract and infer the educational, professional qualifications, and experience requirements for each role, even if they are not directly listed. Please provide this information structured in a JSON format with generic keys such as 'Role', 'EducationalQualifications', 'ProfessionalQualifications', and 'Experience'. This should reflect both clear and deduced criteria to maintain consistency across RFPs with varying levels of detail.

                       The expected response should look like this, filled with either specified or inferred details as per the RFP:
                       {
                        "Role1": {
...
business_context_prompt = ChatPromptTemplate.from_template("""
    ...
    You are official employee of our company tasked with developing a top_notch convincing Proposal. Craft a **Business Context and Relevance** section for our RFP proposal. This section should elucidate the background, underscore the importance of the project to the client, and demonstrate how our company's expertise aligns with the client's needs.
    Please write in a formal and professional tone, as if speaking on behalf of our organization. The language used should be natural, confident, and persuasive. Please utilize the subsequent details:
    ...
    Instead of explicitly stating that certain information is not provided or found, You should elaborate on each section using the provided details.
...
project_understanding_prompt = ChatPromptTemplate.from_template("""
    ...
    Craft a clear and concise response that demonstrates a deep understanding of the project's essence. Ensure the overview connects the requirements, scope, and objectives into a cohesive narrative without explicitly mentioning any missing information.
    The focus is on showcasing our understanding and conviction to the client that we are well-equipped to meet their needs..
...
client_insight_prompt = ChatPromptTemplate.from_template("""
    ...
    You are an employee in our company, you have been tasked to develop a convincing RFP response
      Summarize your understanding of the project expectations based on:
      ...
      **Expected RFP Response Types**: ```{expected_rfp_response}```
      ...
      You should commumicate in formal and more natural but professional tone.
...
work_schedule_prompt = ChatPromptTemplate.from_template("""
        ...
        ```json
        {{
            "Project_Name": "Extract the project name or title based on {objective}",
            "Total_Duration": "Provide a detailed project duration based on **RFP PROJECT DURATION AND KEY MILESTONES",
            "Tasks": [
                {{
...
costing_breakdown_intro_prompt = ChatPromptTemplate.from_template("""
    ...
    Please talk in natural, official and professional tone. Instead of explicitly stating that certain information is not provided or found, You should elaborate on each section using the provided details.
...
software_licensing_table_json_prompt = ChatPromptTemplate.from_template("""
    ...
    Below is an example JSON structure provided for you:
...
#############################################################
generate_resume_prompt = ChatPromptTemplate.from_template("""
                                                          
        Create a detailed dummy resume for the specified role, reflecting a high level of expertise and robust experience suitable for a senior position in an RFP response. The resume should be formatted for easy conversion to LaTeX or markdown for inclusion in the proposal document.

        **INSTRUCTIONS**:
        1. Take the role and its requirements provided in JSON format as input and develop a comprehensive resume that encapsulates the role's qualifications, experience, and education.
        2. Begin with a professional summary that showcases a fictional candidate's extensive expertise and perfect fit for the role's demands.
        3. Construct a work history section with fictional job titles, companies, time periods, and detailed job responsibilities and accomplishments.
        4. Align the educational background and professional certifications with the role's requirements to establish the candidate's suitability.
        5. Include a skills and technologies section to demonstrate the candidate's depth of knowledge and relevant capabilities.
        6. Include other relevant sections that might be important.
        7. Use proper formatting with headers, bullet points, and clear separation of sections to facilitate the translation into LaTeX or markdown.
        

        **ROLES AND REQUIREMENTS FROM RFP Documents**:
        ```{roles_with_qualifications_json}``
        ...
        **DUMMY RESUME FORMAT**:
        ...
        {{
          "{{role name}}": {{
            ...
            "Summary": "Professional summary highlighting experience, expertise, and alignment with the role's requirements.",
            "Professional Experience": [
              {{
                "Job Title": "Applicable job title",
                "Company Name": "Company name",
                "Dates of Employment": "Employment dates",
                "Responsibilities": [
                  "Key responsibility",
        ...
        Fill in the sections marked with placeholders (e.g., role name, role, educational_qualifications, experience, etc) with the relevant information provided for the role. Each resume must be unique, tailored, and reflect a candidate of the highest caliber for the role specified.
        
        Remember to add additional sections that might be important.
        
        If No role name is specified in the **ROLES AND REQUIREMENTS FROM RFP Documents**, do no generate a resume.
...
clean_resume_prompt = ChatPromptTemplate.from_template("""
    
    Please review the resumes provided for the project, identifying and editing any repetitive occurrences of names, addresses,phone number,  emails, and school names to ensure each resume maintains unique and valid information. Upon completion, return the revised resumes in JSON format.

    RESUMES: ```{resume_json}```
    
    Note: Ensure that the profile **names, addresses, phone number,  emails, etc** are unique across the JSON  Response.
    """
)


convert_to_latex = ChatPromptTemplate.from_template("""
    
    Convert the Resume from JSON to Latex format.
...
convert_to_markdown = ChatPromptTemplate.from_template("""
    
    Convert the Resume from JSON to Markdown format.
